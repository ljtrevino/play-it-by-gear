...
class LevelEasyScreen(Screen):
    def __init__(self, level, notes, goal_values, gear_values, **kwargs):
        super(LevelEasyScreen, self).__init__(**kwargs)
        ...
        self.options = LevelOptions(level=level, goal_music_seq=self.goal_music_seq, duration=self.goal_music.duration)
    ...
    def _can_add_gear(self, new_gear_type):
        for gear in self.music_gears:
            if gear.type == new_gear_type:
                return False
        return True
    ...
    def on_layout(self, winsize):
        ...
        for center in self.gear_centers:
            ...
            center.update_music_pos(self.music_box_gear_locations[mapped_dic[gear.type]])
        ...
        # update level label
        self.label.center_x = size_dim*1.5
        self.label.center_y = size_dim*3/5
    ...
    def on_touch_down(self, touch):
        ...
        if other_gears:
            ...
            if response and self._check_music_gears():
                for gear in self.music_gears:
                    gear.toggle_rotate()
